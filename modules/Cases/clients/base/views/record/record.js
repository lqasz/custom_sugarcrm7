({
    ...
    initialize: function(options) {
        ...
        var self = this;
            pclSections = app.lang.getAppListStrings('pcl_sections_list');

        self.pclView = options.context.get("action");
        self.collection.on('data:sync:complete', function() {
            app.api.call('GET', 'index.php?entryPoint=getData&PCLData=1&pcl_id='+ self.model.get('id'), null,{
                success: _.bind(function(data) {
                    ...
                    self.model.trigger('addToSelect');
                })
            });
        }, self);
    ...
    render: function() {
        ...
        this.$el.find(".pcl-row").attr("disabled", true);
        this.$el.find(".pcl-subject-remove").addClass("icon-hiden");
        if(this.pclView == "edit") {
            this.$el.find(".not-disabled").removeAttr("disabled");
            if(app.user.id == "144c39bf-ccc3-65ec-2023-5407f7975b91") {
                this.$el.find(".pcl-subject-remove").removeClass("icon-hiden");
            }
        }
    },
    ...
    returnVerificationField: function(dataName, iter) {
        var options = '',
            ...
            verification = responsible.verification,
            optionsObject = {
               "PUSTY": "",
               "TAK": "TAK",
               "NIE": "NIE",
